...
void AEditorPlayer::PickActor(const FVector& PickPosition, bool bMultiSelect) const
{
    ...
    if (Possible)
    {
        AActor* Owner = Possible->GetOwner();

        if (!bMultiSelect)
        {
            Engine->SelectActor(Owner, false);
        }
        else
        {
            if (Engine->GetSelectedActors().Contains(Owner))
            {
                Engine->DeselectActor(Owner);
            }
            else
            {
                Engine->SelectActor(Owner, true);
            }
        }
